...
cat("##\n")
cat("##                               That's fun!\n")
cat("##                Let's plot some maps and a time series of S5P!\n")
cat("##                             ╰( ^o^)╮╰( ^o^)╮\n")
cat("##\n")
cat("##################################################################################\n")
...
inp_path <- "/RemoteSensing/Results/Other/CSV/TROPOMI"
inp_ts_path <- "/RemoteSensing/Results/Other/Plots/TROPOMI/time_series"
out_path <- "/RemoteSensing/Results/Other/Plots/TROPOMI"
sat_path <- "/RemoteSensing/DATA/Sentinel-5P"

var_name_list <- c("NO2", "CH4")   # "CH4" "NO2"
...
aoi_keys <- c(
    "AOI_BO_City" = "/users/r/b/rbichler/RemoteSensing/DATA/GEODATA/SHP/mapc_dissolved.shp"
)
...
plot_map_time_series <- function(inp_path, map_file_pattern, ts_file_pattern, aoi, var_name, time_res, polygon_path, output_folder) {

    map_l <- list.files(path = inp_path, pattern = map_file_pattern, recursive = FALSE, full.names = TRUE, include.dirs = TRUE)
    ts_l <- list.files(path = inp_ts_path, pattern = ts_file_pattern, recursive = FALSE, full.names = TRUE, include.dirs = TRUE)
    sat_l <- list.files(path = sat_path, recursive = TRUE, full.names = TRUE, include.dirs = TRUE)
    ...
    for (map_file in map_l) {
        ...
        ts_data <- read_csv(ts_file) %>% mutate(date = as.Date(date))
        ...
        for (i in seq(1, length(ts_data$date))) {
            ...
            d <- ts_data$date[i]
            d1 <- gsub("-", ".", d)
            cat(paste0("Processing date: ", d, "\n"))

            ## Retrieve satellite data based on 
            sat_file <- grep(paste0("CONUS_S5P_OFFL_L3_", var_name, "_mm_", d), sat_l, value = TRUE)
            sat_raster <- terra::rast(sat_file)
            
            ## Invert mask to get outside polygon only
            outer_mask <- terra::mask(sat_raster, polygon, inverse = TRUE)
            inner_mask <- terra::mask(sat_raster, polygon)

            ## Retrieve EPSG code
            epsg_code <- terra::crs(sat_raster, describe = TRUE)$code
            
            raster_df <- as.data.frame(sat_raster, xy = TRUE)
            colnames(raster_df) <- c("x", "y", "value")

            raster_df <- as.data.frame(sat_raster, xy = TRUE)
            colnames(raster_df) <- c("x", "y", "value")

            inner_mask_df <- as.data.frame(inner_mask, xy = TRUE)
            ...
            if (var_name == "NO2") {
                ...
                ## Copy the value column to a new column for mol/m2
                raster_df$value_mol_m2 <- raster_df$value

                ## Convert mol/m2 to molecules/cm2
                cat("Convert TROPOMI NO2 values from mol/m2 to molec./cm2\n")
                raster_df$value <- raster_df$value * (6.022 * (10^19))
                inner_mask_df$value <- inner_mask_df$value * (6.022 * (10^19))
                outer_mask_df$value <- outer_mask_df$value * (6.022 * (10^19))

                ## Check if the file contains "orig" or "ansc"
                ...
                outer_mask_df <- outer_mask_df %>% mutate(value = ifelse(value < 0, NA, value))

                print(head(raster_df, 10))
                ...
                v_name <- bquote("Monthly Mean TROPOMI TVC of NO"[2] ~ "[" * .(unit_dic[[var_name]]) * "]")
                l_theme <- element_text(size = 12)

            } else if (var_name == "CH4") {
                ...
                limits <- c(1650, 2100)
                breaks <- c(1650, 1700, 1750, 1800, 1850, 1900, 1950, 2000, 2050, 2100)

                v_name <- bquote("Monthly Mean TROPOMI VC of CH"[4] ~ "[" * .(unit_dic[[var_name]]) * "]")
                l_theme <- element_text(size = 12, angle = 45, margin = margin(t = 12, b = 10))
            }
            
            ## Define colors
            colors <- c("green", "#bdbdbd", "magenta", "#08306B", "#5ea9ff", "#FFFF00", "#FFA500", "#FF0000", "#800080")
            ...
            rescaled_positions <- (color_positions - limits[1]) / diff(limits)
            ...
            labs(
                ...
                caption = paste0(
                    ...
                    "Google Earth Engine; EPSG: ", epsg_code,
            ...
            scale_x_date(
                ...
                limits = c(min(ts_data$date) - 15, max(ts_data$date) + 15),
            ...
            ## Create filename
            fname1 <- gsub("\\d{4}-\\d{2}-\\d{2}-\\d{4}-\\d{2}-\\d{2}", d, fname)

            ## Check if folder name exists and if not create it
            f_path <- file.path(output_folder, paste0(var_name, "/maps/", folder_name))
            qa_var <- "animation"

            if (!dir.exists(file.path(f_path))) {
                dir.create(file.path(f_path), recursive = TRUE)
            } else {cat("Folder already exists!\n")}

            ggsave(
                filename = file.path(f_path, paste0(fname1, "_map.png")),
                plot = full_plot,
                width = 10,
                height = 12,
                bg = "transparent"
            )
            cat(paste0("Saved to: ", file.path(f_path, paste0(fname1, "_map.png\n"))))

            ggsave(
                filename = file.path(f_path, paste0(fname1, "_map_table.png")),
                plot = p_table,
                width = 4,
                bg = "transparent"
            )
            cat(paste0("Saved to: ", file.path(f_path, paste0(fname1, "_map_table.png\n"))))

            ## Create a list of images for animation
            image_files <- c(image_files, file.path(f_path, paste0(fname1, "_map.png")))
        }
        ...
        ## Write out as GIF
        image_write(animation, path = file.path(output_folder, paste0("animations/", fname, "_", qa_var, ".gif")))
        cat(paste0("Saved to: ", file.path(output_folder, paste0("animations/", fname, "_", qa_var, ".gif\n"))))
    }
}
...
for (aoi in aoi_l) {
    for (var_name in var_name_list) {
        for (time_res in time_resolution) {
            ...
            df_map_plot_pattern <- paste0("^", instrument, "_", var_name, "_", time_res, "_TVC_\\d{4}-\\d{2}-\\d{2}-\\d{4}-\\d{2}-\\d{2}_crop_", aoi, "_ID_\\d{+}.csv$")
            df_ts_plot_patttern <- paste0("^", instrument, "_", var_name, "_", time_res, "_TVC_\\d{4}-\\d{2}-\\d{2}-\\d{4}-\\d{2}-\\d{2}_crop_mean_", aoi, "_ID_\\d{+}_plot_df.csv$")

            polygon <- aoi_keys[[aoi]]

            ## Create map and time series
            plot_map_time_series(inp_path, df_map_plot_pattern, df_ts_plot_patttern, aoi, var_name, time_res, polygon, out_path)
        }
    }
}
